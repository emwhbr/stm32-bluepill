...
// internal driver state and synchronization
static volatile SemaphoreHandle_t xTxDoneSem = NULL;

static TaskHandle_t xINTWorkTask = NULL;

static uint8_t *g_rx_packet = NULL;
...
static void enc28j60_enable_ext_int(void)
{
   ...
   // create the worker task for external interrupts generated by ENC28J60
   if (xINTWorkTask == NULL)
   {
      xTaskCreate(enc28j60_int_work_task, "INTWRK", 250, NULL, configMAX_PRIORITIES-1, &xINTWorkTask);
   }
...
void exti15_10_isr(void)
{
   if (exti_get_flag_status(ENC28J60_GPIO_PIN_INT_EXTI) == ENC28J60_GPIO_PIN_INT_EXTI)
   {
      ...
      vTaskNotifyGiveFromISR(xINTWorkTask, &xHigherPriorityTaskWoken);
...
static void enc28j60_int_rx(void)
{
   ...
   while (pkt_cnt-- > 0)
   {
      enc28j60_hw_rx(g_rx_packet, ENC28J60_ETH_MAX_FRAME_SIZE, &actual);
      printf("RX:%u\n", actual); fflush(stdout);
      enc28j60_hex_dump("RX-data", g_rx_packet, actual);
   }
}
...
void enc28j60_init(void)
{
   ...
   if (g_rx_packet == NULL)
   {
      g_rx_packet = pvPortMalloc(ENC28J60_ETH_MAX_FRAME_SIZE);
   }
